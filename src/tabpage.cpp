...
bool ProxyFilter::filterAcceptsRow(const Fm::ProxyFolderModel *model,
                                   FmFileInfo *info) const {
  if (!model || !info)
    return true;
  QString baseName(fm_file_info_get_name(info));
  if (!virtHiddenList_.isEmpty() && !model->showHidden() &&
      virtHiddenList_.contains(baseName))
    return false;
  if (!filterStr_.isEmpty() &&
      !baseName.contains(filterStr_, Qt::CaseInsensitive))
    return false;
  return true;
}
...
TabPage::TabPage(FmPath *path, QWidget *parent)
    : QWidget(parent), folder_(NULL), folderModel_(NULL),
  ...
  connect(proxyModel_, &ProxyFolderModel::sortFilterChanged, this,
          &TabPage::onModelSortFilterChanged);
  ...
  connect(folderView_, &View::openDirRequested, this,
          &TabPage::onOpenDirRequested);
...
void TabPage::freeFolder() {
  if (folder_) {
    g_signal_handlers_disconnect_by_func(folder_,
                                         (gpointer)onFolderStartLoading, this);
    g_signal_handlers_disconnect_by_func(folder_,
                                         (gpointer)onFolderFinishLoading, this);
    g_signal_handlers_disconnect_by_func(folder_, (gpointer)onFolderError,
                                         this);
    g_signal_handlers_disconnect_by_func(folder_, (gpointer)onFolderFsInfo,
                                         this);
    g_signal_handlers_disconnect_by_func(folder_, (gpointer)onFolderRemoved,
                                         this);
    g_signal_handlers_disconnect_by_func(folder_, (gpointer)onFolderUnmount,
                                         this);
...
/*static*/ void TabPage::onFolderStartLoading(FmFolder *_folder,
                                              TabPage *pThis) {
  if (!pThis->overrideCursor_) {
    // FIXME: sometimes FmFolder of libfm generates unpaired "start-loading" and
    // "finish-loading" signals of uncertain reasons. This should be a bug in
    // libfm. Until it's fixed in libfm, we need to workaround the problem here,
    // not to override the cursor twice.
...
void TabPage::restoreScrollPos() {
  ...
  folderView_->childView()->verticalScrollBar()->setValue(
      browseHistory().currentScrollPos());
}

/*static*/ void TabPage::onFolderFinishLoading(FmFolder *_folder,
                                               TabPage *pThis) {
  // FIXME: is this needed?
  FmFileInfo *fi = fm_folder_get_info(_folder);
  if (fi) { // if loading of the folder fails, it's possible that we don't have
            // FmFileInfo.
  ...
  // UI update for performance reasons. Therefore at this point the UI is not up
  // to date. Of course, the scrollbar ranges are not updated yet. We solve this
  // by installing an Qt timeout handler.
  QTimer::singleShot(10, pThis, SLOT(restoreScrollPos()));
}

/*static*/ FmJobErrorAction TabPage::onFolderError(FmFolder *_folder,
                                                   GError *err,
                                                   FmJobErrorSeverity severity,
                                                   TabPage *pThis) {
  if (err->domain == G_IO_ERROR) {
    if (err->code == G_IO_ERROR_NOT_MOUNTED &&
        severity < FM_JOB_ERROR_CRITICAL) {
      FmPath *path = fm_folder_get_path(_folder);
      MountOperation *op = new MountOperation(pThis);
      op->mount(path);
      if (op->wait()) { // blocking event loop, wait for mount operation to
                        // finish.
  ...
  if (severity >= FM_JOB_ERROR_MODERATE) {
     ...
     * This fixes bug #3411298- Show "Permission denied" when switching to super
     * user mode.
     * https://sourceforge.net/tracker/?func=detail&aid=3411298&group_id=156956&atid=801864
     * */

    // FIXME: consider replacing this modal dialog with an info bar to improve
    // usability
...
/*static*/ void TabPage::onFolderFsInfo(FmFolder *_folder, TabPage *pThis) {
  ...
  if (fm_folder_get_filesystem_info(_folder, &total, &free)) {
    ...
    /*
    fm_file_size_to_str(total_str, sizeof(total_str), total,
    fm_config->si_unit);
    ...
    */
    msg = tr("%1 available")
              .arg(QString::fromUtf8(free_str)); // probono: Simplified
  } else
    msg.clear();
...
/*static */ void TabPage::onFolderContentChanged(FmFolder *_folder,
                                                 TabPage *pThis) {
  /* update status text */
  pThis->statusText_[StatusTextNormal] = pThis->formatStatusText();
  Q_EMIT pThis->statusChanged(StatusTextNormal,
                              pThis->statusText_[StatusTextNormal]);
}
...
void TabPage::chdir(FmPath *newPath, bool addHistory) {
  ...
  g_signal_connect(folder_, "start-loading", G_CALLBACK(onFolderStartLoading),
                   this);
  g_signal_connect(folder_, "finish-loading", G_CALLBACK(onFolderFinishLoading),
                   this);
  ...
  g_signal_connect(folder_, "content-changed",
                   G_CALLBACK(onFolderContentChanged), this);
...
void TabPage::onSelChanged(int numSel) {
  ...
  if (numSel > 0) {
    ...
    if (numSel == 1) { /* only one file is selected */
      ...
      if (numSel == 1) {
        /* If only one item is selected, detailed information about that file is
        shown, so that this should actually never be needed */
...
void TabPage::onModelSortFilterChanged() { Q_EMIT sortFilterChanged(); }
