...
QString getLaunchableExecutable(FmFileInfo *_info) {
  ...
  // In this case we should return "nothing", but since apparently we have to
  // return something we return the directory itself
  // TODO: Implement
  QString launchableExecutable =
      QString(fm_path_to_str(fm_file_info_get_path(_info)));
  ...
  if (path.toLower().endsWith(".app")) {
    ...
    // macOS .app bundle
    // TODO: Before falling back to foo.app/Contents/MacOS/foo, parse Info.plist
    // and get the CFBundleExecutable from there
    QFile executableFile2(path.toUtf8() + "/Contents/MacOS/" +
                          nameWithoutSuffix);
    if (executableFile2.exists() && QFileInfo(executableFile2).isExecutable() &&
        QSysInfo::productType() == "osx") {
      launchableExecutable =
          QString(path + "/Contents/MacOS/" + nameWithoutSuffix);
    }
  }
